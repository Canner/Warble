...
export function emitAgentModule(prepared: PreparedDispatch, opts: EmitOptions = {}): string {
  ...
  if (prepared.dependencies.length > 0) {
    throw new DispatchError(
      "emit does not support component_invocation (wall-hit): a generated single-plan run() cannot " +
        "carry the immutable prepared-callee registry, trusted active-step alias binding, or root budget ledger. " +
        "Use the Agent SDK dispatch() runtime for composed entries.",
    );
  }
