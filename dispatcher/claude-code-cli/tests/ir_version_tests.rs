...
/// compat window, and its TS manifest port) and for `core`'s emitted `"warble_ir_version": "..."`
/// literal, all of which are plain string constants rather than arrays. Still asserts the count
/// rather than assuming it, so a future widening to a range is caught the same way
/// `SUPPORTED_IR_VERSIONS` is above, instead of only checking `[0]` and missing an extra element.
/// `needle` must be specific enough to skip past this file's own doc comments (e.g.
