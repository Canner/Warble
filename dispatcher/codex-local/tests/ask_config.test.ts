...
test("renders one isolated custom-agent layer per IR step", () => {
  ...
  const bundle = createOrchestrateAgentConfigBundle(prepared);
  try {
    ...
    for (const [index, agent] of bundle.agents.entries()) {
      ...
      assert.equal(toml, renderOrchestrateAgentToml(prepared, step, bundle.requestFile, bundle.stepRequestFile));
...
test("child instructions require a structured step envelope and forbid fallback surfaces", () => {
  ...
  for (const step of prepared.steps) {
    const toml = renderOrchestrateAgentToml(prepared, step, "/tmp/fake-warble-request", "/tmp/fake-warble-step");
...
test("get_context receives the authoritative question argument", () => {
  ...
  const toml = renderOrchestrateAgentToml(prepared, resolveIntent, "/tmp/fake-warble-request", "/tmp/fake-warble-step");
  assert.match(
    toml,
...
test("run_sql agents must guard aggregate cardinality", () => {
  ...
  const toml = renderOrchestrateAgentToml(prepared, generateSql, "/tmp/fake-warble-request", "/tmp/fake-warble-step");
