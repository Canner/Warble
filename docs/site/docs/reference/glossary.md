...
The load-bearing terms. See `ir-schema` for the IR contract, `capability-model` for how
required capabilities resolve against a runtime target, and `component-composition` for the
...
| **Component call** | A step-scoped, same-profile invocation authorized by `llm_steps[].component_calls`. The structured alias-to-mount edge grants authority; `prompt_ref` decides when, how often, and with what request to use it. The compiler emits it in v0.8; the Agent SDK and composed Codex local orchestrate targets realize the read-only first slice with their documented runtime budgets; unsupported shapes and other targets wall-hit. |
| **Context binding** | What a profile is pointed at. `kind` is required — there is no default. `kind: prepared` reads a document the host already resolved into Warble's own projection (metrics, dimensions, grains, lineage, impact), named by a separate `document` field, which is how any semantic format binds without Warble speaking it; `kind: raw_source` uses `RawSourceContext` for constitutive schema/document probes; `kind: external` deliberately introspects nothing. The coarse authored locator is retained in every case. |
| **MDL** | Modeling Definition Language — the semantic-layer format (models, metrics, dimensions, relationships) a wren project declares. One of the formats a host may own; Warble does not read it, and reaches it only as a prepared-context document (see Context binding above). |
