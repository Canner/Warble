...
guardrail (`capability-model` §7.1). A missing or malformed document fails loudly: the binding
...
- `attestation_gate` — a terminal action may proceed only on a **fresh, passing attestation**
  ...
  [`enforcement-seam`](/reference/enforcement-seam#7-attestation_gate--declared-scored-offline-not-yet-enforced)
...
and `threshold` are passthrough (see [`ir-schema`](/reference/ir-schema)), so a new guardrail's
