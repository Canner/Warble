...
| `--context-injection <mode>` | *(claude-code target only)* Which normalized context is embedded in emitted prompts. `schema-only` (default) is currently the only value: a deterministic schema digest built from the compiled IR. The mode names a normalized context facet, not a context provider, and an unrecognized value fails before any output is written. |
