...
lives at `Canner/Warble`; `.github/workflows/eval.yml` runs the committed jaffle smoke baseline on
manual dispatch only, since every run spends real model calls. It skips cleanly without the
`CLAUDE_CODE_OAUTH_TOKEN` secret instead of producing a credential-related false failure. The long-term bottleneck stays golden-truth generation (curate → capture-confirmed →
