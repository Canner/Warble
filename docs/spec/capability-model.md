...
[decision on lineage ownership](./blast-radius.md#1-ownership-split). So that argument now justifies
...
### 7.3 `component_invocation` — same-profile calls
...
The Agent SDK target declares this capability `native` through its dispatcher-owned isolated-child
handler. It exposes only the active step's aliases as ephemeral in-process tools, starts every
