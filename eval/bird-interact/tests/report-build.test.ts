...
import { BIRD_COMPONENT_ID, DISALLOWED_BUILT_INS } from "../src/envelope.js";
...
test("every run is labelled a degraded-config measurement", () => {
  // Unconditional by design: the collapse happens in the agent's constructor, so there is no
  // per-run input that could turn this off. `tests/agent.test.ts` pins the collapse itself, so a
  // future differentiated binding fails there rather than leaving this sentence describing
  // something that stopped happening.
  const r = buildRunReport(inputs());
  const warning = r.warnings.find((w) => /degraded-config/i.test(w));
  assert.ok(warning, "no degraded-config label");
  assert.match(warning, /strong, cheap and orchestrator/);
});

test("every run states what the fixed envelope does not exercise", () => {
  const r = buildRunReport(inputs());
  const warning = r.warnings.find((w) => /does not exercise/i.test(w));
  assert.ok(warning, "no envelope-scope statement");
  // These loops are deliberately TAUTOLOGICAL and that is the point: the warning is generated by
  // joining the same constants, so text and runtime move together and drift is not expressible.
  // Verified by mutation -- adding an entry to DISALLOWED_BUILT_INS leaves this test passing,
  // because the sentence grew the entry too. What the test actually guards is that the statement
  // exists at all and is built from the constants rather than hand-written; deleting the warning
  // fails it. A hand-written list is what would need a drift guard, and this is why there is none.
  for (const builtIn of DISALLOWED_BUILT_INS) assert.ok(warning.includes(builtIn), builtIn);
  assert.ok(warning.includes(BIRD_COMPONENT_ID));
  assert.match(warning, /one component/);
});
