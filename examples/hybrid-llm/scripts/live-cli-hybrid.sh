...
"$WB" dispatch "$work/answer_query.ir.json" --target claude-code:headless \
  ...
  --hybrid-realization "$REAL" --out "$OUT" 1>&2
echo "   emitted:" 1>&2
find "$OUT" -type f | sed "s#$OUT/#     #" 1>&2

mkdir -p "$PROJ/.claude/agents"
cp "$OUT/.claude/agents/answer_query.md" "$PROJ/.claude/agents/answer_query.md"
cleanup() { rm -f "$PROJ/.claude/agents/answer_query.md"; }
trap cleanup EXIT

TRACE="$OUT/hybrid-trace.jsonl"; rm -f "$TRACE"
cd "$PROJ"
...
if [ "$REAL" = "mcp-server" ]; then
  out=$(PATH="$PROJ/.venv/bin:$PATH" env -u ANTHROPIC_BASE_URL claude -p "$Q" \
    --agent answer_query --mcp-config "$OUT/.mcp.json" \
...
echo
echo "generated artifacts kept at: $OUT  (override with OUT=<dir> to keep elsewhere)"
