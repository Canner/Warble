...
| `live-cli-hybrid.sh` | **CLI (file) target hybrid — skill-shell realization.** `warble dispatch` emits a driver + a local-inference script; the driver (`claude -p --agent`) runs the script for the local step and does the SQL itself on cloud Opus. The file-target counterpart of `live-m2.sh`. | ollama + Claude login; `cargo build --release -p warble-cli` | `$1` = question |
