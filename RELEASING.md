...
6. Merge the release PR as an ordinary merge (release-please's own commit message already
   ...
   - Triggers `.github/workflows/release.yml` (cargo-dist) on the `vX.Y.Z` tag push only. Its
     trigger pattern requires a literal leading `v` specifically so an `ir-spec-vX.Y.Z` tag push
     does not also spin up a Cargo/binary release for a TypeScript-only component — see the comment
     above that trigger line in the workflow file for the full reasoning and the alternative
     (cargo-dist's `tag-namespace` config) that was evaluated and rejected. cargo-dist then builds
...
- Hand-push a tag like `v0.5.0-beta.1` expecting a safe, non-publishing dry run. It still matches
  `.github/workflows/release.yml`'s tag trigger (the trailing `*` in `v[0-9]+.[0-9]+.[0-9]+*` matches
...
### Manual edits to cargo-dist-generated files

`.github/workflows/release.yml` is generated by `dist init` / `dist generate` and is normally
safe to regenerate and overwrite. One line in it is hand-edited and documented in place rather than
left to regeneration: the tag-trigger pattern under `on.push.tags`. See the comment directly above
that line in the workflow file for what it does, what breaks if the edit is lost on a future
regeneration, and the `tag-namespace` alternative that was evaluated and rejected in its favor.
Re-apply that edit (or restore it from git history) after any future `dist init`/`dist generate`.
