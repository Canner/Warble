...
The IR (`warble_ir_version`, currently `0.8`) is a **separate version line from the crate/package
...
Both dispatchers declare a `peerDependencies` range on `@warble/ir-spec` (`0.8.x` today) plus an
advisory `"warble": { "irVersion": "0.8" }` field — so the IR a published dispatcher speaks is
